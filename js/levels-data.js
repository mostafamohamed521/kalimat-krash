// ============================================
// KALIMAT CRASH — levels-data.js
// بيانات 100 مستوى — نمط ثابت للمودات
// ============================================
...
// كل كلمة: word, clue, row, col, dir ('h'=أفقي, 'v'=رأسي)
// التقاطعات محققة — buildCrosswordGrid تتحقق وقت التشغيل وتتجاهل التعارضات

const CROSSWORD_DATA_M4 = [

    // ===== مستوى 1: ملح، مدرسة، حمام، رسالة، سلم =====
    // م@(0,0): ملح[0] = مدرسة[0] = م ✓
    // ح@(0,2): ملح[2] = حمام[0]  = ح ✓
    // ر@(2,0): رسالة[0] = مدرسة[2] = ر ✓
    // ا@(2,2): رسالة[2] = حمام[2]  = ا ✓
    // س@(3,0): سلم[0] = مدرسة[3] = س ✓
    {
        words: [
    ...
    // ===== مستوى 2: تفاح، تمساح، حمص، فانوس، خوخ =====
    // ت@(0,0): تفاح[0] = تمساح[0] = ت ✓
    {
        words: [
    ...
    // ===== مستوى 3: شمس، سماء، ضوء، مطر، ثلج =====
    // س@(0,2): شمس[2] = سماء[0] = س ✓
    // باقي الكلمات مستقلة بلا تعارض
    {
        words: [
    ...
    // ===== مستوى 4: قلم، قراءة، ليمون، دفتر، علم =====
    // ق@(0,0): قلم[0] = قراءة[0] = ق ✓
    // ل@(0,1): قلم[1] = ليمون[0] = ل ✓
    {
        words: [
    ...
    // ===== مستوى 5: أسد، أرنب، سمكة، ذئب، فيل =====
    // أ@(0,0): أسد[0] = أرنب[0] = أ ✓
    // س@(0,1): أسد[1] = سمكة[0] = س ✓
    {
        words: [
    ...
    // ===== مستوى 6: طبيب، طائرة، بيت، بحر، نهر =====
    // ط@(0,0): طبيب[0] = طائرة[0] = ط ✓
    // ب@(0,3): طبيب[3] = بيت[0] = ب ✓
    {
        words: [
    ...
    // ===== مستوى 7: قهوة، قمر، وردة، شاي، لبن =====
    // ق@(0,0): قهوة[0] = قمر[0]  = ق ✓
    // و@(0,2): قهوة[2] = وردة[0] = و ✓
    {
        words: [
    ...
    // ===== مستوى 8: سيارة، سفينة، يختار، قطار، مترو =====
    // س@(0,0): سيارة[0] = سفينة[0] = س ✓
    // ي@(2,0): سفينة[2] = يختار[0] = ي ✓
    // ا@(2,2): يختار[2] = سيارة[2] = ا ✓
    // ر@(2,3): يختار[3] = سيارة[3] = ر ✓
    {
        words: [
    ...
    // ===== مستوى 10: فضاء، فلك، ضياء، لياء، مجرة =====
    // ف@(0,0): فضاء[0] = فلك[0]  = ف ✓
    // ض@(0,1): فضاء[1] = ضياء[0] = ض ✓
    // ل@(1,0): فلك[1]  = لياء[0] = ل ✓
    // ي@(1,1): ضياء[1] = لياء[1] = ي ✓
    {
        words: [
    ...
    // ===== مستوى 11: رياضة، ركض، يرمي، كرة، بطل =====
    // ر@(0,0): رياضة[0] = ركض[0]  = ر ✓
    // ي@(0,1): رياضة[1] = يرمي[0] = ي ✓
    // ك@(1,0): ركض[1]   = كرة[0]  = ك ✓
    // ر@(1,1): يرمي[1]  = كرة[1]  = ر ✓
    {
        words: [
    ...
    // ===== مستوى 12: موسيقى، مغني، غناء، عزف، أغنية =====
    // م@(0,0): موسيقى[0] = مغني[0] = م ✓
    // غ@(1,0): مغني[1]   = غناء[0] = غ ✓
    {
        words: [
    ...
    // ===== مستوى 13: طعام، طبخ، عنب، خبز، زيت =====
    // ط@(0,0): طعام[0] = طبخ[0] = ط ✓
    // ع@(0,1): طعام[1] = عنب[0] = ع ✓
    // خ@(2,0): طبخ[2]  = خبز[0] = خ ✓
    // ب@(2,1): عنب[2]  = خبز[1] = ب ✓
    {
        words: [
    ...
    // ===== مستوى 14: حديقة، حياة، دوار، نبات، ماء =====
    // ح@(0,0): حديقة[0] = حياة[0] = ح ✓
    // د@(0,1): حديقة[1] = دوار[0] = د ✓
    {
        words: [
    ...
    // ===== مستوى 15: مدرسة، مريم، رسم، سبورة، كتاب =====
    // م@(0,0): مدرسة[0] = مريم[0] = م ✓
    // ر@(1,0): مريم[1]  = رسم[0]  = ر ✓
    {
        words: [
...
// هذه الدالة تتحقق أن جميع تقاطعات الكلمات صحيحة وتعيد بيانات معدّلة
function validateAndFixCrossword(data) {
    ...
    for (const wInfo of words) {
        ...
        // إذا كانت هناك تعارض، نتجاهل الكلمة (مع الاحتفاظ ببقية الكلمات)
    }
...
//   مستوى 3 → مود 3
//   مستوى 4 → مود 4
//   من مستوى 5 فصاعداً يتكرر النمط:
//     3 × مود 1  ثم  2 × مود 2  ثم  1 × مود 3  ثم  1 × مود 4
//     (دورة واحدة = 7 مستويات)
//
// الخوارزمية المستخدمة:
//   [Round-Robin Pattern] — توزيع دوري ثابت لا يتغير بين الجلسات

function buildLevelList() {
    // [Round-Robin Pattern]: نبني مصفوفة المودات الـ 100 بالترتيب الثابت
    const pattern = [];

    // أول 4: واحد من كل مود (تعريفي)
    pattern.push(1, 2, 3, 4);

    // من مستوى 5: نمط دوري ثابت 3-2-1-1
    let i = 4;
    while (i < 100) {
        // [Round-Robin chunk]: 3×M1, 2×M2, 1×M3, 1×M4 = 7 مستويات لكل دورة
